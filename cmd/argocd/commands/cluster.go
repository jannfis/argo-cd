...
func NewClusterAddCommand(clientOpts *argocdclient.ClientOptions, pathOpts *clientcmd.PathOptions) *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(c *cobra.Command, args []string) {
			...
			if awsClusterName != "" {
				...
				errors.CheckError(err)
				managerBearerToken, err = common.InstallClusterManagerRBAC(clientset, systemAccountNamespace)
				errors.CheckError(err)
			}
	...
	command.Flags().StringVar(&systemAccountNamespace, "sa-namespace", "kube-system", "Create required SystemAccount in different namespace, defaults to kube-system")
	return command
}
